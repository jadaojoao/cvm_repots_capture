        ...
        <nav className="hidden items-center gap-1 md:flex">
          {PRIMARY_NAV.map((item) =>
            item.href ? (
              ...
              <span
                key={item.label}
                className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[0.84rem] text-muted-foreground"
              >
                {item.label}
                <Badge
                  variant="outline"
                  className="rounded-full border-border/80 bg-background/70 text-[0.68rem] font-medium uppercase tracking-[0.18em] text-muted-foreground"
                >
                  Em breve
                </Badge>
              </span>
