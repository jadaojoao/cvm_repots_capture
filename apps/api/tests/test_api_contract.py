...
def test_companies_empty_search_returns_default_list(client: TestClient):
    response = client.get("/companies")

    assert response.status_code == 200
    payload = response.json()
    assert len(payload) == 2
    assert payload[0]["company_name"] == "PETROBRAS"
    assert payload[0]["anos_disponiveis"] == [2023, 2024]


def test_companies_search_filters_results(client: TestClient):
    response = client.get("/companies", params={"search": "vale", "limit": 20})

    assert response.status_code == 200
    payload = response.json()
    assert len(payload) == 1
    assert payload[0]["company_name"] == "VALE"
...
def test_service_failure_returns_503(client: TestClient):
    ...
    class BrokenService:
        ...
        def search_companies(self, search: str = ""):
